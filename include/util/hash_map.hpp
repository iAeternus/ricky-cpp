...
/**
 * @class HashMap
 * @brief 哈希表，提供高效的键值对存储、检索和更新功能
 * @details 使用分桶和Robin哈希来处理冲突，并通过动态调整桶的大小来保持高效的负载因子
...
class HashMap : Object<HashMap<K, V, Alloc, Bucket>> {
public:
    using key_t = K;         // 键的类型
    using value_t = V;       // 值的类型
    using bucket_t = Bucket; // 桶的类型
    using Self = HashMap<key_t, value_t, Alloc, bucket_t>;

    /**
     * @brief 默认构造函数
     * 初始化一个空哈希表，使用指定的桶大小（默认为 MIN_BUCKET_SIZE）
     * @param bucket_size 桶的初始大小
     */
    HashMap(usize bucket_size = MIN_BUCKET_SIZE) :
     ...
     */
    HashMap(std::initializer_list<Pair<key_t, value_t>>&& init_list) :
            HashMap(roundup2(init_list.size() / MAX_LOAD_FACTOR)) {
    ...
    value_t& get(const _K& key) {
        ...
        if (value == nullptr) {
            throw not_found_exception("Key '{}' not found in hash_map", key);
        }
        return *value;
    }
    ...
    /**
     * @brief 向哈希表中插入键值对，如果键已存在，则覆盖原有值
     * @param key 键
     * @param value 值
    ...
    /**
     * @brief 向哈希表中插入键值对，如果键已存在，则覆盖原有值（提供哈希值版本）
     * @param key 键
     * @param value 值
    ...
    /**
     * @brief 计算两个哈希表的并集，键相同则选择其他哈希表的值
     * @param other 另一个哈希表
     * @return 返回两个哈希表的并集
     */
    Self operator|(const Self& other) const {
     ...
     * @brief 哈希表迭代器类
     */
    class HashMapIterator : public Object<HashMapIterator> {
    public:
         ...
         */
        HashMapIterator() :
                hash_map_(nullptr), index_(0), kv_() {}
         ...
         * @param index 初始索引
         */
        HashMapIterator(const HashMap* map, usize index) :
                hash_map_(map), index_(index) {
            update_kv(0);
        }

        /**
         * @brief 拷贝构造函数
         * @param other 需要拷贝的哈希表迭代器
         */
        HashMapIterator(const Self& other) = default;
        ...
        Self operator+(difference_type n) const {
            return Self(hash_map_, index_ + n);
        }
    ...
    private:
        ...
        void update_kv(i64 add) {
            index_ += add;
            if (index_ < 0 || index_ >= hash_map_->size()) {
                kv_.set(nullptr, nullptr);
                return;
            }
            const key_t& key = hash_map_->keys_[index_];
            kv_.set(&key, &hash_map_->get(key));
        }
    ...
    using const_iterator = HashMapIterator; // 常量哈希表迭代器类型
...
private:
     ...
     * @brief 扩展桶的大小
     * @details
     * 当哈希表的负载因子（键值对的数量除以桶的容量）超过最大阈值 `MAX_LOAD_FACTOR` 时，
...
/**
 * @brief 判断类型是否为 HashMap
 */
template <typename>
struct is_hash_map : std::false_type {};

template <typename K, typename V>
struct is_hash_map<util::HashMap<K, V>> : std::true_type {};

template <typename T>
constexpr bool is_hash_map_v = is_hash_map<T>::value;
