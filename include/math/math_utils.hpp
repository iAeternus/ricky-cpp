...
template <typename T>
auto min_(const T& a, const T& b) -> const T& {
    return b < a ? b : a;
}

template <typename T>
auto max_(const T& a, const T& b) -> const T& {
    return a < b ? b : a;
}
...
auto gcd(const T& a, const T& b) -> T {
 ...
 */
template <IntegerType T>
auto lcm(const T& a, const T& b) -> T {
    return a / gcd(a, b) * b;
}
 ...
 */
template <IntegerType T>
auto mul(const T& a, const T& b, T p = MOD) -> T {
 ...
 */
template <IntegerType T>
auto pow_mod(T a, T n, T p = MOD) -> T {
    T res{1};
...
auto fcmp(const T& a, const T& b, T eps = EPS) -> cmp_t {
    if (a - b >= eps) {
        return 1;
    } else if (b - a >= eps) {
        return -1;
 ...
 */
template <FloatingPointType T>
auto is_pos(const T& num, T eps = EPS) -> bool {
 ...
 */
template <FloatingPointType T>
auto is_neg(const T& num, T eps = EPS) -> bool {
    return fcmp(num, 0.0, eps) < 0;
}
 ...
 */
template <FloatingPointType T>
auto is_zero(const T& num, T eps = EPS) -> bool {
    return fcmp(num, 0.0, eps) == 0;
}
 ...
 */
template <FloatingPointType T>
auto is_one(const T& num, T eps = EPS) -> bool {
