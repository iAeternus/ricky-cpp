...
void should_mse_loss_forward() {
    ...
    FTensor input = FTensor::arange(0, 4).view(Shape{2, 2});  // [[0,1],[2,3]]
    FTensor target = FTensor::ones(Shape{2, 2});               // [[1,1],[1,1]]

    FTensor loss = mse.forward(input, target);
...
void should_cross_entropy_loss_forward() {
    ...
    FTensor input = FTensor::arange(0, 6).view(Shape{2, 3});
    FTensor target = FTensor(Shape{2}, static_cast<f32>(0));  // [0, 0]

    // Should not throw and produce a scalar loss
    FTensor loss = cel.forward(input, target);
