 ...
 * @details 1. 该分配器使用全局的 new 和 delete 操作符进行内存管理，支持对象的构造和析构
 *          2. 适用于需要自定义内存管理的场景，如 STL 容器的自定义分配器
 * @note 该分配器不支持线程安全，使用时需确保线程安全性
 * @tparam T 分配的对象类型
 */
template <typename T>
class Allocator {
public:
     ...
     * @tparam U 其他类型的分配器
     * @example
     * Allocator<int>::rebind<double>::other
     */
    template <typename U>
    struct rebind {
    ...
    /**
     * @brief 分配内存
     * @details 1. 分配n个value_type类型对象的内存，不会调用对象的构造函数
     *          2. 如果n为0，则返回nullptr；如果n超过最大可分配大小，则抛出 std::bad_alloc 异常
     *          3. 使用全局的 new 操作符进行内存分配
     * @param n 要分配的对象数量
    ...
    /**
     * @brief 释放内存
     * @details 1. 释放p指向的内存块，n为要释放的对象数量
     *          2. 如果p为nullptr，则不执行任何操作
     *          3. 使用全局的 delete 操作符进行内存释放
     * @param p 指向要释放内存的指针
     * @param n 要释放的对象数量
     */
    ...
    /**
     * @brief 构造对象
     * @details 1. 在p指向的内存位置构造一个value_type类型的对象，使用给定的参数
     *          2. 如果p为nullptr，则不执行任何操作
     *          3. 使用placement new进行对象构造
     * @tparam U 要构造的对象类型
    ...
    /**
     * @brief 析构对象
     * @details 1. 在p指向的内存位置析构一个value_type类型的对象，n为要析构的对象数量
     *          2. 如果p为nullptr，则不执行任何操作
     *          3. 使用对象的析构函数进行析构
     * @tparam U 要析构的对象类型
     ...
     * @details 1. 在分配的内存中构造一个value_type类型的对象，使用给定的参数
     *          2. 如果分配失败，则返回nullptr，并且在异常情况下会自动释放内存
