 ...
 */
template <Hashable K>
auto my_hash_impl(const K& key, std::false_type) -> hash_t {
 ...
 */
template <Hashable K>
auto my_hash(const K& key) -> hash_t {
 ...
 * @return 解码后的32位无符号整数
 */
inline auto decode_fixed32(const char* ptr) -> u32 {
 ...
 * @return 计算得到的哈希值
 */
inline auto bytes_hash(const char* data, const size_t n, const u32 seed = 0xbc9f1d34) -> hash_t {
